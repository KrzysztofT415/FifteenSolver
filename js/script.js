...
const ModifiedHeuristic = tiles => {
    let distance = 0, neighbour_distance = 0
    for (let i = 0; i < 16; ++i) {
        let tile = parseInt(getTile(tiles, BigInt(i))) - 1
        if (tile < 0) tile = 15
        distance += Math.abs(i % 4 - tile % 4) + Math.abs((i / 4 | 0) - (tile / 4 | 0))
...
let swapTiles = (tiles, index1, index2) => {
    let shift1 = (15n - index1) * 4n, shift2 = (15n - index2) * 4n
    let tile1 = tiles >> shift1 & 15n, tile2 = tiles >> shift2 & 15n
...
const randomBoardTiles = moves => {
    let tiles = BigInt('0x123456789abcdef0'), zero = 15, next = null, prev = null
...
let getNeighbours = (index, parent_zero_index) => [index - 1, index - 4, index + 1, index + 4]
    .filter(neighbour => neighbour >= 0 && neighbour < 16 && Math.abs(neighbour % 4 - index % 4) <= 1 && neighbour !== parent_zero_index)
...
const checkBoardParity = tiles => {
    let inv = 0, tiles_array = []
    for (let i = 0; i < 16; ++i) {
        tiles_array[i] = tiles >> (15n - BigInt(i)) * 4n & 15n

        if (tiles_array[i] !== 0n) {
            for (let j = 0; j < i; ++j)
                if (tiles_array[j] > tiles_array[i])
                    ++inv
